...
public class BearPairs
{
    ...
    {
        var n = s.Length;
        var G = Enumerate(2 * n + 2, x => new List<Edge<int, int>>());
        for (int i = 0; i < n; i++)
        {
            G.AddDirectedEdge(n, i, 1, 0);
            G.AddDirectedEdge(i + n, n + 1, 1, 1);
            for (int j = 0; j < n; j++)
            {
                if (s[i] != s[j])
                {
                    G.AddDirectedEdge(i, i + n, 1, Math.Abs(j - i) * 100 + cost[i] + cost[j]);
                }

            }
        }
        var res = Flow.PrimalDual(G, n, n + 1, (n - k) / 2);
        Debug.WriteLine(res);
        return res.x;
    }
